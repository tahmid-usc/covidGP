...
covid <- covid %>% filter(state == "South Carolina") %>%  
  mutate(t = (as.numeric(date) - mindate)/100, y = casess / max(casess))


plot(covid$t, covid$y)
plot(covid$date, covid$casess)
...
plot(datestar, posmu, lwd = 3, type = 'l', col = 2, main = 'Cumulative COVID-19 casess Prediction for South Carolina', xaxt = 'n', ylim = c(0, max(ul)), xlab = "Day", ylab = 'Cumulative casess')
